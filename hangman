...
class Hangman():
    ...
    def find_indexes(self, letter):
        ...
        """
        return [i for i, char in enumerate(self.word_to_guess) if letter in char] 
        # 입력된 알파벳(letter)이 맞추고자하는 단어 안에 있는 알파벳이다
        # 그러면 그 알파벳이 맞추고자 하는 단어에 인덱스들을 반환
        # 예를 들어 casa가 정답인 경우, a를 입력하면 [1,3] 을 반환해주는 것이 이 메소드의 기능이다.
        # (letter,i)로 하지 않아도 충분히 구현 가능하다. 입력된 글자와 인덱스를 묶어서 반환하면 나중에 a에 해당하는 인덱스 불러올때 까다롭다.
    ...
    def is_invalid_letter(self, input_):
        ...
        """
        return (input_.isdigit()) or (input_.isalpha() and len(input_)>=2)
